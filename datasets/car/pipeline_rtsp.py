...
class RTSPPipeline:
    ...
    def __init__(
        ...
        # VLLM OCR 클라이언트 초기화 (서버는 한 번만 시작)
    ...
    def _initialize_model(self):
        ...
        if self.model is None:
            self.logger.info(f"YOLO 모델 로드 중: {self.tracking_model_path}")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="RTSP 실시간 차량 데이터셋 구축 파이프라인",
...
예제:
  ...
  python -m datasets.car.pipeline_rtsp \\
    ...
    parser.add_argument(
        ...
        help="저장할 동영상 파일 경로 (기본값: rtsp_output.mp4)"
    )
